...
//  Lambini
//
//  Created by Alex Yu on 12/1/2016.
//  Copyright © 2016 XMind. All rights reserved.
//
...
class Line: NSObject {
    ...
    func drawInContext(context: CGContext, isDebuggingEnabled: Bool, usePreciseLocation: Bool) {
        ...
        for point in points {
            ...
            // warning: line width
            CGContextSetLineWidth(context, (point.magnitude - 2) * 2)
...
class LinePoint: NSObject  {
    ...
    let minMagnitude: CGFloat = 3.0
    let maxMagnitude: CGFloat = 5.0

    var magnitude: CGFloat {
        return min(max(force, minMagnitude), maxMagnitude)
    }
